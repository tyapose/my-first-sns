...
def groups(request):
    ...
    if request.method == 'POST':
        ...
        if request.POST['mode'] == '__friends_form__':
            ...
            messages.success(request, ' チェックされたＦｒｉｅｎｄを' +sel_group + 'に登録しました。')
...
def add(request):
    ...
    if add_user == request.user:
        messages.info(request, "自分自身をＦｒｉｅｎｄに追加することは出来ません")
    ...
    messages.success(request, add_user.username + 'を追加しました！'\
           ' groupページに移動して、追加したＦｒｉｅｎｄをメンバーに設定してください。')
    return redirect(to='/sns')
...
@login_required(login_url='/aadmin/login')
...
def good(request,good_id):
    ...
    if is_good > 0:
        messages.success(request,'既にメッセージにはｇｏｏｄしてます。')
        return redirect(to='/sns')
    ...
    messages.success(request,'メッセージにＧｏｏｄしました！')
    return redirect(to='/sns')
...
def get_your_group_message(owner,glist,find):
    ...
    if find==None:
        ...
        messages = Message.objects.filter(Q(group__in=groups)\
                    ...
                    .filter(content__conteins=find)[:100]
